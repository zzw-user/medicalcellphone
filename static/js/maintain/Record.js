...
function assess(id) {
	mui.prompt('请输入你对此的评估', '不错', '评估', ['取消', '确认'], function(e) {
		if (e.index == 1) {
			mui.ajax(localStorage.getItem('adminurl')+'/zAmend/updRecord',{
			    data:{
...
function list(id){
	mui.ajax(localStorage.getItem('adminurl')+'/zAmend/getRecordOne',{
	    data:{
			rid:id
	    },
	    dataType: 'json', //服务器返回json格式数据
...
function del(id){
	mui.confirm('确定要删除此数据吗，确认？', 'Hello MUI', ['取消','确认'], function(e) {
		if (e.index == 1) {
			mui.ajax(localStorage.getItem('adminurl')+'/zAmend/delRecord',{
			    data:{
					rid:id
			    },
			    dataType: 'json', //服务器返回json格式数据
...
function select(param){
	mui.ajax(localStorage.getItem('adminurl')+'/zAmend/getRecord',{
	    data:param,
	    dataType: 'json', //服务器返回json格式数据
	        type: 'post', //HTTP请求类型
	        timeout: 10000, //超时时间设置为10秒；
